...
public:
    ...
    bool insertSymbol(std::string , std::string) ;
...
bool ScopeTable::insertSymbol(SymbolInfo* symbol) {
    ...
    if(ptr) {
        //Already exists
        std::cout << "\t" << symbol << " already exists in the current ScopeTable" << std::endl;
        return false ;
    }
...
bool ScopeTable::insertSymbol(std::string name , std::string type) {
    if(lookupHelper(name)) {
        std::cout << "\t" << "'"<< name <<"'" << " already exists in the current ScopeTable" << std::endl;
        return false ;
    }
    return insertSymbol(new SymbolInfo(name , type)) ;
}
...
void ScopeTable::printScopeTable() {
    ...
    for(size_t i = 0 ; i< table_size ; i++) {
        ...
        while(cur) {
            std::cout << " <" << cur->getName() << "," << cur->getType() << ">" ;
            cur = cur->nxt ;
        }
    std::cout << std::endl ;
