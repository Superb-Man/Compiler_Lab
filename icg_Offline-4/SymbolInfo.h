...
public:
    ...
    std::vector<SymbolInfo*>trueList ;
    std::vector<SymbolInfo*>falseList ;
    std::vector<SymbolInfo*>nxtList ;
    std::string label ;
    bool global = false ;
    size_t arr_strt = 0 ;
    std::string asm_name ="" ;
    ...
    std::string get_asm_name(int idx) {
        if(global == true) {
            return asm_name + "[SI+" + to_string(idx)+"]" ;
        }
        size_t offset = arr_strt + 2 * idx ;
        return "W. [BP-"+to_string(offset)+"]" ;
    }
    ...
    SymbolInfo(SymbolInfo* symbol) {
        ...
        this->var = symbol->var ;
        this->arr_strt = symbol->arr_strt ;
        this->global = symbol->global ;
        this->label = symbol->label ;
        this->asm_name = symbol->asm_name ;
